...
| A working tool starts receiving HTML error pages | `SSL_CERT_FILE` or `REQUESTS_CA_BUNDLE` exported from a shell profile, replacing the trust store for every program | `env \| grep -E 'SSL_CERT\|CA_BUNDLE'` | Remove them from the profile and use per-tool configuration; see [Installation](installation.md). Test with `env -u SSL_CERT_FILE -u REQUESTS_CA_BUNDLE <command>`. |
